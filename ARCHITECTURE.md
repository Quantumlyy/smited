...
The "one backend per vendor" template (OWO Skin) and "one backend per physical device, all sharing one SDK singleton" template (bHaptics) are the two patterns this codebase ships, and new vendors should copy whichever fits:

- **Single-device-singleton (OWO).** One vendor → one descriptor kind (`owo_skin`) → one factory → one backend instance owning one SDK singleton talking to one paired device through one local helper app (MyOWO). Per-vendor lifecycle: the backend's `ConnectAsync` opens the SDK transport; its `DisposeAsync` tears it down. `Smited.Daemon.Owo` follows this shape.
- **Shared-SDK-multi-device (bHaptics).** One vendor → multiple descriptor kinds (`bhaptics_vest`, `bhaptics_sleeve_l`, `bhaptics_sleeve_r`, `bhaptics_feet_l`, `bhaptics_feet_r`) → one factory class registered N times in DI (one instance per kind, kind passed in the constructor because `IBackendFactory.Kind` is a single string) → N backend instances → one shared SDK singleton because the vendor's helper app (bHaptics Player) owns every paired device for the host. Per-vendor lifecycle: the SDK's `InitializeAsync` is idempotent across every backend that touches it; the backends never dispose the SDK singleton — the host DI container does at shutdown. Each backend's heartbeat polls `Sdk.IsDeviceConnected(DeviceKey)` per its own device, independent of the others. `Smited.Daemon.Bhaptics` follows this shape; PiShock and any other multi-device-per-vendor backend should clone it.
...
A second pattern lives alongside the per-component subscription model: `BodyMapPageState` (`Admin/Services/`) is a daemon-wide singleton `IHostedService` that subscribes to the `EventBus` once at host startup and maintains per-(backend, zone) activity. Blazor components on `/bodymap` inject it and listen to its `StateChanged` event, so all connected admin clients see the same activity map and a single bus subscription serves every visitor. Mirror this shape (singleton + hosted-service alias) when a new admin surface needs cross-page state that outlives any one Blazor circuit — see `HistoryEventBusSubscriber` for the same pattern in the history pipeline.
...
`Smited.Daemon.Bhaptics.csproj` follows the same Windows-conditional template as OWO: `net9.0-windows` TFM, `Bhaptics.Tac` NuGet (1.4.2) gated on `_TargetingWindows`, SDK-touching files (`BhapticsBackendBase.cs` and the three concrete backends, `BhapticsBackendFactory.cs`, `StaticBhapticsSdk.cs`) excluded from compile on non-Windows hosts. `BackendsServiceCollectionExtensions.AddBhapticsBackendIfWindows` does the same `Type.GetType` reflective load plus a foreach over the five kind constants (`bhaptics_vest`, `bhaptics_sleeve_l/r`, `bhaptics_feet_l/r`) registering one `IBackendFactory` instance per kind via `ActivatorUtilities.CreateInstance(sp, factoryType, kind)`. `IBhapticsSdk` and the bhaptics options classes live in `Smited.Daemon.Abstractions`. The motor-map and zone-topology data tables (`BhapticsMotorMap.cs`, `BhapticsZoneTopology.cs`, `BhapticsBackendParameters.cs`) also live in `Smited.Daemon.Abstractions` so the mock backends in `Smited.Daemon.Backends.Mock` produce bit-identical payloads to the real backends on Windows.
