...
`Smited.Daemon.Bhaptics.csproj`, by contrast, targets plain `net9.0`. The bHaptics integration uses BCL `System.Net.WebSockets.ClientWebSocket` and has no Windows-only build dependencies, so the assembly is platform-portable and the daemon takes a normal compile-time `ProjectReference`. `BhapticsBackendFactory` is registered alongside the cross-platform mocks in `BackendsServiceCollectionExtensions.AddSmitedBackends`; the factory's own `TryCreate` enforces the runtime Windows-only constraint (bHaptics Player only runs on Windows) by returning `null` on non-Windows hosts. Cross-platform compilation pays off in test coverage: the protocol DTOs, `PlayerClient`, and `ZoneIndexMap` get full unit-test coverage on Mac CI against a Kestrel-backed simulator, leaving only the on-hardware smoke test as Windows-bound.
