...
internal static class BackendsServiceCollectionExtensions
{
    ...
    /// <see cref="MockOwoBackendFactory"/> alongside the existing
    /// <see cref="MockOwoBackend"/> singleton, plus the bHaptics
    /// factories: <see cref="MockBhapticsBackendFactory"/> for the
    /// in-process simulator and <see cref="BhapticsBackendFactory"/>
    /// for the real Player-backed backend (the latter declines on
    /// non-Windows at <c>TryCreate</c> time).
    /// </summary>
    public static IServiceCollection AddSmitedBackends(this IServiceCollection services)
    {
        ...
        services.TryAddEnumerable(
            ServiceDescriptor.Singleton<IBackendFactory, MockBhapticsBackendFactory>());
        services.TryAddEnumerable(
            ServiceDescriptor.Singleton<IBackendFactory, BhapticsBackendFactory>());
        return services;
    }
